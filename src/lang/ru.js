const messages = {
  msg: {
    ...
    welcome: 'Добро пожаловать в Kepler кошелек',
    ...
    back: 'отступать',
    login: {
      walletExist: 'Данные Kepler кошелька найдены; Войти с оригинальным паролем :-)',
    },
    ...
    app:{
      ...
      updateMsg: 'Найдена новая версия Kepler кошелка. Обновить прямо сейчас?',
      yes: 'Да',
      no: 'Нет',
    ...
    httpSend:{
      ...
      salteVersionHelp: 'Если вам не удалось отправить Kepler, попробуйте изменить версию Slate файла'
    },
    ...
    hedwig: {
      ...
      introTitle: 'Введение',
      intro1: 'Hedwig(v1) представляет из себя ретранслятор для пользователей без публичного IP, предоставляя временный адрес для получения Kepler.',
      intro2: 'Когда кто-то отправляет Kepler на адрес, Hedwig(v1) перешлет отправку запроса на ваш кошелек. Таким образом, вы получите свои Kepler.',
      start: 'Запустить',
      ...
      failed3: 'Ошибка при запуске локального сервиса получения Kepler, попробуйте позже или перезапустите кошелек.',
    ...
    check: {
      ...
      intro2: "По этой причине Kepler предоставляет команду проверки вручную, которая сканирует набор UTXO-цепочки для любых выходов, принадлежащих вашему кошельку, и обеспечивает их соответствие с вашей локальной базой данных кошелька.",
